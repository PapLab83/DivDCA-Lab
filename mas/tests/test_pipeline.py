...
class TestProcessTickerSuccess:
    ...
    def test_returns_list(self, container_with_success_agent, single_ticker_data):
        results = process_ticker(
            container_with_success_agent,
            single_ticker_data["ticker"],
            single_ticker_data["records"],
        )
        assert isinstance(results, list)

    def test_result_count_matches_records(self, container_with_success_agent, single_ticker_data):
        results = process_ticker(
            container_with_success_agent,
            single_ticker_data["ticker"],
            single_ticker_data["records"],
    ...
    def test_all_results_successful(self, container_with_success_agent, single_ticker_data):
        results = process_ticker(
            container_with_success_agent,
            single_ticker_data["ticker"],
            single_ticker_data["records"],
    ...
    def test_result_contains_required_fields(self, container_with_success_agent, single_ticker_data):
        results = process_ticker(
            container_with_success_agent,
            single_ticker_data["ticker"],
            single_ticker_data["records"],
    ...
    def test_ticker_and_year_propagated(self, container_with_success_agent, single_ticker_data):
        results = process_ticker(
            container_with_success_agent,
            single_ticker_data["ticker"],
            single_ticker_data["records"],
    ...
    def test_data_contains_agent_output(self, container_with_success_agent, single_ticker_data):
        results = process_ticker(
            container_with_success_agent,
            single_ticker_data["ticker"],
            single_ticker_data["records"],
    ...
    def test_duration_is_non_negative(self, container_with_success_agent, single_ticker_data):
        results = process_ticker(
            container_with_success_agent,
            single_ticker_data["ticker"],
            single_ticker_data["records"],
...
class TestProcessTickerFailure:
    ...
    def test_all_results_failed(self, container_with_fail_agent, single_ticker_data):
        results = process_ticker(
            container_with_fail_agent,
            single_ticker_data["ticker"],
            single_ticker_data["records"],
        )
        for r in results:
    ...
    def test_error_contains_exception_info(self, container_with_fail_agent, single_ticker_data):
        results = process_ticker(
            container_with_fail_agent,
            single_ticker_data["ticker"],
            single_ticker_data["records"],
    ...
    def test_failure_does_not_stop_processing(self, container_with_fail_agent, single_ticker_data):
...
class TestProcessTickerPartialFailure:
    ...
    def test_mixed_results(self, container_with_partial_fail_agent):
        records = [
        ...
        results = process_ticker(
            container_with_partial_fail_agent, "MIX", records,
        )
    ...
    def test_successful_results_have_data(self, container_with_partial_fail_agent):
        records = [
...
class TestProcessTickerEdgeCases:
    ...
    def test_empty_records_returns_empty(self, container_with_success_agent):
        results = process_ticker(container_with_success_agent, "EMPTY", [])
        assert results == []

    def test_single_record(self, container_with_success_agent, single_record):
        results = process_ticker(
            container_with_success_agent, "SINGLE", [single_record],
        )
        assert len(results) == 1
        assert results[0]["success"] is True

    def test_negative_yoy_change(self, container_with_success_agent):
        records = [{"year": 2022, "price": 50, "dividend": 1, "yoy_change": -46.6}]
        results = process_ticker(
            container_with_success_agent, "NEG", records,
        )
        assert len(results) == 1
        assert results[0]["success"] is True

    def test_zero_dividend(self, container_with_success_agent):
        records = [{"year": 2022, "price": 100, "dividend": 0.0, "yoy_change": -100.0}]
        results = process_ticker(
            container_with_success_agent, "ZERO", records,
        )
        assert len(results) == 1
...
class TestProcessTickerIntegration:
    ...
    def test_full_pipeline_single_record(self, integration_container):
        records = [{"year": 2021, "price": 150.0, "dividend": 3.0, "yoy_change": 10.0}]
        results = process_ticker(integration_container, "INTG", records)
    ...
    def test_full_pipeline_multiple_records(self, integration_container):
        records = [
        ...
        results = process_ticker(integration_container, "AAPL", records)

        assert len(results) == 3
        assert all(r["success"] for r in results)
        assert all(r["duration_ms"] >= 0 for r in results)

    def test_full_pipeline_with_mock_tickers(self, integration_container, mock_tickers_data):
    ...
    def test_prompt_version_is_set(self, integration_container):
        records = [{"year": 2021, "price": 100, "dividend": 2, "yoy_change": 5}]
        results = process_ticker(integration_container, "VER", records)
