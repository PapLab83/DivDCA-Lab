...
class AgentRegistry:
    ...
    def __init__(self):
        self._agents: Dict[str, AgentMetadata] = {}  # ← исправлен тип
        self._load_defaults()
    ...
    def validate_agent(self, agent_type: str, config: AgentConfig) -> bool:
        ...
        if metadata.supported_providers:
            if config.llm_config.provider.value not in metadata.supported_providers:
                logger.warning(
                    f"Агент {agent_type} не поддерживает "
                    f"провайдера {config.llm_config.provider}"
                )
                return False
        return True